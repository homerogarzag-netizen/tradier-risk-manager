...
st.set_page_config(layout="wide", page_title="Trading Command Center", page_icon="🛡️")
...
with st.sidebar:
    ...
    if st.button("🗑️ Limpiar Gráficos"):
        ...
        st.rerun()
    st.caption("v16.0.0 | Full Unified Suite")
...
def run_master_analysis():
    ...
    # 3. Ganancias Realizadas (Oficial del Broker)
    ...
    spy_ret = clean_df(spy_df)['Adj Close'].pct_change().dropna()
    ...
    return {
        "nl": float(r_p.json()['profile']['account']['total_equity']) if isinstance(r_p.json()['profile']['account'], dict) else float(r_p.json()['profile']['account'][0]['total_equity']),
        "rd": total_rd, "bwd": total_bwd, "th": total_th, "lev": total_exp, "risk": t_map,
        "detailed": detailed_positions, "gl": gl_data, "spy_p": spy_p
    }
        ...
        if d:
            ...
            with tab_risk:
                ...
                if len(h) > 1:
                    ...
                    g1, g2 = st.columns(2)
                    with g1: st.write("**Evolución Capital**"); st.area_chart(h, x="Timestamp", y="Net_Liq")
                    ...
                    if not longs.empty:
                        ...
                        for gl in d['gl']:
                            ...
                            if u_sym == und and o_type == "CALL":
                                ...
                                # Solo sumamos si el trade se cerró después de comprar el LEAPS
                                if close_dt >= start_date:
                                    # Evitamos sumar el propio LEAPS si se cerró una parte
                        ...
                        st.markdown(f'<div class="section-header">ACTIVO: {und} (Spot: ${group["Price"].iloc[0]:.2f})</div>', unsafe_allow_html=True)
                        cc1, cc2, cc3, cc4 = st.columns(4)
                        cc1.markdown(f'<div class="pmcc-summary-card"><small>COSTO LEAPS</small><br><b>${l_cost:,.2f}</b></div>', unsafe_allow_html=True)
                        cc2.markdown(f'<div class="pmcc-summary-card"><small>CC REALIZADO</small><br><b style="color:#4ade80">${realized_income:,.2f}</b></div>', unsafe_allow_html=True)
                        cc3.markdown(f'<div class="pmcc-summary-card"><small>NET INCOME</small><br><b>${net_inc:,.2f}</b></div>', unsafe_allow_html=True)
                        cc4.markdown(f'<div class="pmcc-summary-card"><small>ROI TOTAL</small><br><b style="color:#4ade80">{roi:.1f}%</b></div>', unsafe_allow_html=True)
                        
                        st.caption("🏛️ Core Positions (LEAPS)")
                        st.table(longs[['Exp', 'Strike', 'Qty', 'Delta']])
                        
                        # Monitor Jugo Short Activo
                        ...
                        if closed_trades:
                            with st.expander("📔 Ver Historial de Ventas Cerradas"):
                                st.table(pd.DataFrame(closed_trades).sort_values("Fecha", ascending=False))
                        st.divider()

            with st.expander("Ver Detalle Crudo"): st.dataframe(df_det)
else:
    st.info("👈 Ingresa tu Token de Tradier para activar el Command Center.")
